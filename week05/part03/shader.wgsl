...
fn main_fs(
  ...
  // use OBJ vertex color as base diffuse color
  let base = color.xyz;

  // material terms
  let k_d = base * uniforms.k_d;
  let k_a = k_d;
  let k_s = vec3f(1.0) * uniforms.k_s;

  // fixed directional light direction
  let omega_i = normalize(vec3f(0.0, 0.0, 1.0));

  // light radiance (white)
  let L_e = vec3f(1.0) * uniforms.L_e;
  let L_a = vec3f(1.0) * uniforms.L_a;
