struct Uniforms {
    ...
    k_s: f32,
    s: f32,
    // Align to 16 bytes: 3 floats + 1 float + 1 float + 1 float + 1 float + 1 float
    // Total floats: 3 + 5 = 8 floats = 32 bytes (aligned)
    mvp: mat4x4f,
}
...
@vertex
fn main_vs(@location(0) inPos: vec4f, @location(1) color: vec4f, @location(2) normal: vec4f, @builtin(instance_index) instance: u32) -> VSOut {
    var vsOut: VSOut;
...
@fragment
fn main_fs(@location(0) inPos: vec4f, @location(1) color: vec4f, @location(2) normal: vec4f) -> @location(0) vec4f {
    ...
    // Light direction (Fixed light from viewer direction effectively)
    ...
    // eye position comes from uniform which is updated by quaternion rotation
